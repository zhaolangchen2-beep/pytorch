...
struct GuardSubtreeCodeAccessorProof {
  ...
  bool matches_current() const {
    return PyFunction_GET_CODE(function.ptr()) == code.ptr();
  }
};
...
struct GuardSubtreeAttrOwnerProof {
  ...
  bool matches_current(PyObject* current_self) const {
    ...
    return dict.ptr() != nullptr && dictptr != nullptr &&
        *dictptr == dict.ptr() &&
...
static bool guard_last_success_add_attr_type_binding(
  ...
  if (type_proof == nullptr) {
    ...
    type_proof = &proofs.back();
  } else if (
      type_proof->getattr != type->tp_getattr ||
      type_proof->getattro != type->tp_getattro ||
      type_proof->version != type->tp_version_tag) {
    return false;
  }
  type_proof->require_default_getattribute |= require_default_getattribute;

  for (const auto& binding : type_proof->bindings) {
    if (binding.key.ptr() == key.ptr()) {
      return binding.expected.ptr() == expected;
    }
  }
  GuardSubtreeAttrTypeBinding binding;
  binding.key = key;
  if (expected != nullptr) {
    binding.expected = py::reinterpret_borrow<py::object>(expected);
  }
  type_proof->bindings.push_back(std::move(binding));
  return true;
}

static bool guard_last_success_add_attr_owner_proof(
    std::vector<GuardSubtreeAttrOwnerProof>& proofs,
    const GuardActualPartialAccessorRecord& record,
    PyObject* current_self,
    PyObject* expected) {
...
static bool guard_fast_plan_enabled() {
  ...
  // Plan tokens rely on borrowed PyObject pointers remaining stable under GIL.
  return false;
#else
...
struct GuardSubtreeEntryToken {
  ...
  // Owns the exact Tensor instance dict whose unchanged version proves that
  // no_hasattr_key remains absent.
  py::object no_hasattr_dict;
  ...
  static GuardSubtreeEntryToken make_aliasing(
      PyObject* obj,
      const void* guard,
      GuardSubtreeProbeTokenKind kind) {
    TORCH_INTERNAL_ASSERT(
        kind == GuardSubtreeProbeTokenKind::ObjectAliasing ||
        kind == GuardSubtreeProbeTokenKind::NoTensorAliasing);
    GuardSubtreeEntryToken token;
    token.object = obj;
    token.type = Py_TYPE(obj);
    token.kind = kind;
    token.relational_guard = guard;
    return token;
  }

  static bool make_tensor_no_hasattr(
      PyObject* obj,
      PyObject* key,
      GuardSubtreeEntryToken* token) {
    token->object = obj;
    ...
    token->size = PyDict_GET_SIZE(dict);
    if (!guard_actual_partial_reserve_container_items(
            static_cast<size_t>(token->size))) {
      return false;
    }
    ...
    while (PyDict_Next(dict, &pos, &dict_key, &dict_value)) {
      if (!PyUnicode_CheckExact(dict_key)) {
        return false;
      }
    }
    token->no_hasattr_dict = py::reinterpret_borrow<py::object>(dict);
    token->version = get_dict_version_unchecked(dict);
    return true;
  }
...
static bool guard_last_success_make_partial_hot_tokens(
  ...
  for (const auto& token : tokens) {
    if (token.kind == GuardSubtreeProbeTokenKind::TensorNoHasAttr) {
      ...
      if (tensor_token.no_hasattr_key != nullptr &&
          (tensor_token.no_hasattr_key != token.no_hasattr_key ||
           tensor_token.no_hasattr_dict.ptr() != token.no_hasattr_dict.ptr() ||
           tensor_token.version != token.version ||
           tensor_token.size != token.size)) {
        return false;
      }
      tensor_token.no_hasattr_key = token.no_hasattr_key;
      tensor_token.no_hasattr_dict = token.no_hasattr_dict;
      tensor_token.version = token.version;
      tensor_token.size = token.size;
      continue;
    }
    if (token.kind == GuardSubtreeProbeTokenKind::ObjectOnly ||
      ...
      // The plan checks self identity directly, binding proofs cover bound
      // methods, and retained exact tuples cannot change after training.
      continue;
    }
...
static bool guard_subtree_tensor_dict_snapshot_matches_current(
  ...
  if (current_dict != token.no_hasattr_dict.ptr()) {
    return false;
  }
...
static bool guard_last_success_add_absent_type_binding(
    PyTypeObject* type,
    PyObject* lookup_key,
    std::vector<GuardSubtreeAttrTypeProof>& proofs) {
  if (type == nullptr || lookup_key == nullptr ||
  ...
  return guard_last_success_add_attr_type_binding(
      proofs,
      type,
      py::reinterpret_borrow<py::object>(lookup_key),
      nullptr,
      false);
}
...
static bool guard_last_success_build_type_method_proofs(
    const std::vector<GuardActualPartialAccessorRecord>& records,
    PyObject* current_self,
    std::vector<GuardSubtreeAttrOwnerProof>& owner_proofs,
  ...
  for (const auto& record : records) {
    ...
    if (!guard_last_success_add_attr_owner_proof(
            owner_proofs, record, current_self, nullptr)) {
      return false;
    }

    if (!guard_last_success_add_attr_type_binding(
            type_proofs,
            record.owner_type,
            record.key,
            record.resolved.ptr(),
...
static bool guard_last_success_build_instance_attr_proofs(
  ...
  for (const auto& record : records) {
    ...
    if (!guard_last_success_add_attr_owner_proof(
            owner_proofs, record, current_self, record.resolved.ptr())) {
      return false;
    }
...
static bool guard_last_success_build_known_static_attr_proofs(
  ...
  for (const auto& record : records) {
    ...
    PyTypeObject* expected_owner_type =
        module_direct ? &PyModule_Type : &PyType_Type;
    ...
    if (record.owner_dict.ptr() == nullptr ||
        PyDict_GetItem(record.owner_dict.ptr(), record.key.ptr()) !=
            record.resolved.ptr() ||
...
static bool guard_last_success_type_accessors_are_covered(
    ...
    PyObject* current_self,
    const std::vector<GuardSubtreeGenericDictOwnerProof>& generic_dict_proofs,
    const std::vector<GuardSubtreeAttrOwnerProof>& attr_owner_proofs) {
...
static bool guard_last_success_build_code_accessor_proofs(
  ...
  for (const auto& record : records) {
    ...
    if (record.owner.ptr() == nullptr || record.resolved.ptr() == nullptr ||
        ...
        PyFunction_GET_CODE(record.owner.ptr()) != record.resolved.ptr()) {
      PyErr_Clear();
      return false;
    }
...
static bool guard_last_success_build_partial_plan_tokens(
    ...
    std::vector<GuardSubtreeEntryToken>& hot_tokens,
    std::vector<GuardSubtreeAttrTypeProof>& attr_type_proofs,
  ...
  if (!guard_last_success_make_partial_hot_tokens(
  ...
  for (const auto& token : partial_tokens) {
    if (token.kind == GuardSubtreeProbeTokenKind::TensorNoHasAttr &&
        !guard_last_success_add_absent_type_binding(
            token.type, token.no_hasattr_key, attr_type_proofs)) {
      return false;
    }
  }
...
struct GuardLastSuccessPartialPlanPayload {
  ...
  // Non-owning: the CacheEntry-owned root outlives its receipt.
...
struct GuardLastSuccessPartialPlan : GuardLastSuccessPartialPlanPayload {
  ...
  void reset_to(GuardLastSuccessPartialPlanState next_state) {
    ...
    auto retired_stability_tokens =
        std::exchange(stability_tokens, std::vector<GuardSubtreeEntryToken>{});
    stable_passes = 0;
    unstable_passes = 0;
    state = next_state;
    // Retired owning references are released after the replacement state is
    // fully published, so a Python finalizer cannot observe a partial plan.
  }

  void reset() {
    reset_to(GuardLastSuccessPartialPlanState::Training);
  }

  void disable() {
    reset_to(GuardLastSuccessPartialPlanState::Disabled);
  }
  ...
  void observe_successful_training_pass(
    ...
    auto next_stable_passes = stable_passes;
    auto next_unstable_passes = unstable_passes;
    if (stable) {
      next_stable_passes += 1;
    } else {
      if (has_training_signature) {
        next_unstable_passes += 1;
        if (next_unstable_passes >=
      ...
      next_stable_passes = 1;
      next_state = GuardLastSuccessPartialPlanState::Training;
    }
    if (next_state != GuardLastSuccessPartialPlanState::Enabled &&
        next_stable_passes >= kGuardLastSuccessActualStablePasses) {
      next_state = GuardLastSuccessPartialPlanState::Enabled;
      next_unstable_passes = 0;
    }
    ...
    state = next_state;
    // As in reset_to(), old owning references retire only after the new plan
    // is complete and safe for a reentrant lookup to observe.
  }
};
...
static bool guard_subtree_memo_tokens_match(
  ...
  for (const auto& token : tokens) {
    ...
    if (token.kind == GuardSubtreeProbeTokenKind::ExactList) {
      if (!guard_subtree_exact_list_token_matches_current(token)) {
        return false;
      }
      continue;
    }
    // Identity-only, relational, and reachability-only tokens are removed
    // when the hot token vector is built. Unknown future kinds fail closed.
    return false;
  }
  return true;
}
...
static bool guard_last_success_prepare_actual_partial(
  ...
  if (partial_tokens[0].object != current_self ||
      partial_tokens[0].type != Py_TYPE(current_self)) {
    return false;
  }
  ...
  build.self_accessor = self_accessor;
  ...
  build.attr_owner_proofs.clear();
  build.attr_type_proofs.clear();
  if (!guard_last_success_build_type_method_proofs(
          accessor_records,
          current_self,
          build.attr_owner_proofs,
          build.attr_type_proofs)) {
    return false;
  }
...
static bool guard_last_success_actual_partial_tokens_match(
  ...
  for (auto& proof : plan.attr_type_proofs) {
    if (!proof.matches_or_refreshes_current()) {
      return false;
    }
  }
  for (const auto& proof : plan.attr_owner_proofs) {
    if (!proof.matches_current(current_self)) {
      return false;
    }
  }
  ...
  return guard_subtree_memo_tokens_match(plan.tokens, local_state);
}

struct GuardActualPartialRecorder {
  std::vector<GuardSubtreeEntryToken> tokens;
...
thread_local GuardActualPartialRecorder* active_guard_actual_partial_recorder =
    nullptr;

static bool guard_actual_partial_recording_active() {
  return active_guard_actual_partial_recorder != nullptr &&
      active_guard_actual_partial_recorder->supported;
}
...
static bool guard_actual_partial_reserve_container_items(size_t count) {
  if (active_guard_actual_partial_recorder == nullptr) {
    return true;
  }
  auto& recorder = *active_guard_actual_partial_recorder;
  if (!recorder.supported ||
      recorder.container_items > kGuardLastSuccessActualMaxContainerItems ||
      count > kGuardLastSuccessActualMaxContainerItems -
          recorder.container_items) {
    recorder.supported = false;
    return false;
  }
  recorder.container_items += count;
  return true;
}
...
static void guard_actual_partial_record_generic_dict_binding(
    ...
    PyObject* dict,
    const std::string& source) {
  if (!guard_actual_partial_is_recording_source(source) || owner == nullptr ||
      dict == nullptr) {
    return;
  }
  ...
  active_guard_actual_partial_recorder->accessor_records.push_back(
      std::move(record));
}
...
static bool guard_actual_partial_is_recording_source(
    const std::string& source) {
  return guard_actual_partial_recording_active() &&
      (active_guard_actual_partial_recorder->self_depth > 0 ||
...
static void guard_actual_partial_mark_unsupported() {
  if (active_guard_actual_partial_recorder != nullptr &&
      active_guard_actual_partial_recorder->self_depth > 0) {
    active_guard_actual_partial_recorder->supported = false;
  }
}
...
static void guard_actual_partial_record_code_accessor(
    PyObject* parent,
    const std::string& source) {
  if (!guard_actual_partial_is_recording_source(source) || parent == nullptr) {
    return;
  }
...
struct GuardActualPartialSelfScope {
  explicit GuardActualPartialSelfScope(bool enter) {
    if (enter) {
      recorder = active_guard_actual_partial_recorder;
      ++recorder->self_depth;
    }
  }
  ...
  GuardActualPartialRecorder* recorder{nullptr};
};

struct GuardActualPartialRecorderScope {
  explicit GuardActualPartialRecorderScope(GuardActualPartialRecorder& recorder)
      : previous(std::exchange(
            active_guard_actual_partial_recorder, &recorder)) {}

  ~GuardActualPartialRecorderScope() {
    active_guard_actual_partial_recorder = previous;
  }

  GuardActualPartialRecorder* previous{nullptr};
};

static void append_guard_subtree_memo_token(
    GuardSubtreeEntryToken&& token,
    const std::string& debug_path) {
  if (!guard_actual_partial_recording_active()) {
    return;
  }
  auto& recorder = *active_guard_actual_partial_recorder;
  if (recorder.tokens.size() >= kGuardLastSuccessActualMaxTokens) {
    recorder.supported = false;
    return;
  }
  recorder.tokens.emplace_back(std::move(token));
  recorder.debug_paths.push_back(debug_path);
}
...
class LeafGuard {
 public:
  ...
  virtual bool append_subtree_memo_token(PyObject* value) {
...
class NO_HASATTR : public LeafGuard {
 public:
  ...
  bool append_subtree_memo_token(PyObject* value) override {
    ...
    GuardSubtreeEntryToken token;
    if (!GuardSubtreeEntryToken::make_tensor_no_hasattr(
            value, _attr_name.ptr(), &token)) {
      guard_actual_partial_mark_unsupported();
      return true;
    }
    append_guard_subtree_memo_token(
        std::move(token), "<TENSOR_NO_HASATTR>");
    return true;
  }
...
class OBJECT_ALIASING : public RelationalGuard {
 public:
  ...
  bool append_subtree_memo_token(PyObject* value) override {
    ...
    append_guard_subtree_memo_token(
        GuardSubtreeEntryToken::make_aliasing(
            value,
            static_cast<const void*>(this),
            GuardSubtreeProbeTokenKind::ObjectAliasing),
        "<OBJECT_ALIASING>");
    return true;
  }
  ...
  bool append_subtree_memo_token(PyObject* value) override {
    ...
    append_guard_subtree_memo_token(
        GuardSubtreeEntryToken::make_aliasing(
            value,
            static_cast<const void*>(this),
            GuardSubtreeProbeTokenKind::NoTensorAliasing),
        "<NO_TENSOR_ALIASING>");
    return true;
  }
 ...
 public:
  ...
  bool check_nopybind_template(T* value) { // borrowed ref
    if constexpr (std::is_same_v<T, PyObject>) {
      if (C10_UNLIKELY(guard_actual_partial_recording_active())) {
        append_guard_subtree_memo_token(
  ...
  const GuardAccessor* unique_accessor_for_source(
      const std::string& source) const {
    const GuardAccessor* found = nullptr;
    for (const auto& accessor : _accessors) {
    ...
    for (const auto& guard : _leaf_guards) {
      ...
      if constexpr (std::is_same_v<T, PyObject>) {
        ...
        if (C10_UNLIKELY(guard_actual_partial_recording_active()) &&
          ...
          result = guard->append_subtree_memo_token(value);
        } else {
...
class RootGuardManager : public GuardManager {
 public:
  ...
  bool check_nopybind_template(
    ...
    if constexpr (HasActualPartial) {
      ...
      *actual_partial_status = GuardActualPartialCheckStatus::NotAttempted;
      if (actual_partial_plan->self_framelocals_index < 0 ||
          actual_partial_plan->self_accessor == nullptr) {
    ...
    [[maybe_unused]] bool use_actual_partial = false;
    if constexpr (HasActualPartial) {
    ...
    bool accessor_result;
    if constexpr (HasActualPartial) {
      accessor_result = use_actual_partial
          ? GuardManager::check_accessors_nopybind_skipping_accessor(
                value, actual_partial_plan->self_accessor)
          : GuardManager::check_accessors_nopybind(value);
    } else {
      accessor_result = GuardManager::check_accessors_nopybind(value);
    }
    if (!accessor_result) {
      return false;
    }
  ...
  bool check_nopybind_actual_partial(
    ...
    return check_nopybind_template<true>(value, &plan, &status);
  }
...
class TENSOR_MATCH : public LeafGuard {
 public:
  ...
  bool append_subtree_memo_token(PyObject* value) override {
    ...
    append_guard_subtree_memo_token(std::move(token), _tensor_name);
    return true;
  }
...
class DictGetItemGuardAccessor : public GuardAccessor {
 public:
  bool supports_actual_partial_subtree_memo(
    ...
    return PyDict_CheckExact(parent) && _key != nullptr &&
        guard_actual_partial_is_deeply_immutable(_key);
  }
  ...
  bool check_nopybind(PyObject* obj, bool matches_dict_tag = false) override {
    if (matches_dict_tag && _is_immutable_object &&
        !guard_actual_partial_recording_active() &&
...
class CodeGuardAccessor : public GuardAccessor {
 public:
  ...
  bool check_nopybind(PyObject* obj, bool matches_dict_tag = false)
    ...
    PyObject* x = PyFunction_GET_CODE(func); // borrowed ref
    return _guard_manager->check_nopybind(x);
  }
  ...
  GuardDebugInfo check_verbose_nopybind(
    ...
    PyObject* x = PyFunction_GET_CODE(func);
...
bool run_root_guard_manager_with_last_success_receipt(
  ...
  if (C10_UNLIKELY(receipt == nullptr)) {
  ...
  const GuardAccessor* self_accessor =
      root_mgr->unique_accessor_for_source(self_source);
  const int self_framelocals_index =
      self_accessor == nullptr ? -1 : self_accessor->framelocals_index();
  if (self_framelocals_index < 0) {
    plan->disable();
    return run_root_guard_manager(root, f_locals);
  }

  GuardActualPartialRecorder recording;
  {
    GuardActualPartialRecorderScope recorder(recording);
  ...
  if (!guard_last_success_prepare_actual_partial(
          ...
          self_accessor,
          recording.tokens,
          recording.debug_paths,
          recording.accessor_records,
          build)) {
    plan->disable();
    return true;
  }
